...
def voters(request):
    ...
    if request.method == 'POST':
        if userForm.is_valid() and voterForm.is_valid():
            ...
            voter.token = jwt.encode(payload, settings.SECRET_KEY, "HS256")
            voter.path = views.generate_stega_images(voter)
...
def deleteVoter(request):
    ...
    try: 
        ...
        file_ = settings.MEDIA_ROOT + "/image_otp/otp/"+ base64.b64encode(admin.email.strip().encode()).decode()
        file1 = "%s1.png" % (file_)
        file2 = "%s2.png" % (file_)

        # add code to check if the platform is a Linux or a Windows Machine
        os.system("rm -fr %s %s" % (file1, file2))
        admin.delete()
