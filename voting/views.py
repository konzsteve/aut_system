...
from .models import Position, Candidate, Voter, Votes, ImageOTP
...
import requests
import json
from stega.asd import _decode as decode
...
def dashboard(request):
    user = request.user
    # check if the user is verified by the admin to vote
    if user.voter.verified and not user.voter.voted:
        # this redirected to provide the image provided for you during registration fuck whoever reads this
        return redirect(reverse("verify_otp"))
        #return redirect(reverse("show_ballot"))
    elif user.voter.verified and user.voter.voted:
        context = {"my_votes": Votes.objects.filter(voter=user.voter)}
        return render(request, "voting/voter/result.html", context)
    else:
        # this is when the user is not verified to voter
        #return HttpResponse("You are not verified to vote so izzah")
        messages.error(request, "Cannot Perform this action! Ask Admin for Verification")
...
def dashboard(request):
    ...
    # check if the user has voted or if they are verified to vote
    if user.voter.verified == True 
...
def decode_image(user):
    import os
    file = settings.OTP_PATH + user.voter.path
    file_ = settings.OTP_PATH + base64.b64encode(user.email.strip().encode()).decode()
    file2_ = "%s2.png" % file_
    imge = user.voter.upload
    file1_ = settings.MEDIA_ROOT[:-6]+ imge.otp_image.url

    
    token_ = decode(file1_, file2_)

    if token_ == user.voter.token.split(".")[1]:
        return True
    else:
        return False
...
def verify_otp(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if decode_image(request.user):
                messages.success(request, "Congratulations! You can successfully vote")
                return redirect(reverse("show_ballot"))
            else:
                messages.error(request, "Provide a valid image")
        
    form = ImageUploadForm()
    context = {"form": form, "title": "Verify OTP"}
    return render(request, "voting/voter/verify.html", context) 
